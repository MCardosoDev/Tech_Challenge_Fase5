...
class IDA_f2Table:
    ...
    def add_data(self, df):
        ...
        try:
            for _, row in df.iterrows():
                ...
                session.add(ida)
            session.commit()
            # print("Dados de IDA_f2 adicionados com sucesso.")
        except Exception as e:
            # print(f"Erro ao adicionar dados de IDA_f2: {e}")
            session.rollback()
        finally:
            session.close()
    ...
    #     '''
    #     fundamental_table = IDA_f2Table(self.engine)
    ...
    #     print(f"Tabela 'IDA.Fundamental2' criada no banco de dados. Inseridos '{len(df_fundamental_renomeado)}' registros")